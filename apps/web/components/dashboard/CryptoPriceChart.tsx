    ...
    async function loadChart() {
      ...
      try {
        const response = await fetch(
          `/api/crypto/chart?coin=${selectedCoin.id}&days=${days}`,
          {
    ...
    const coordinates = points.map(
      (point, index) => {
        ...
        const normalizedPrice =
          (point.price - minimumPrice) /
          priceRange;

        const y =
          chartHeight -
          chartPadding -
          normalizedPrice * drawableHeight;
  ...
  return (
    <section className="mb-12 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:p-8">
      <div className="flex flex-wrap items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Compare recent cryptocurrency price
            movement.
          </p>
        </div>
          ...
          <div className="flex h-80 items-center justify-center rounded-2xl bg-slate-50">
            <div className="text-center">
              ...
              <p className="mt-4 text-sm font-semibold text-slate-500">
                Loading {selectedCoin.name} chart…
              </p>
            </div>
          </div>
        )}
      ...
      <p className="mt-5 text-xs text-slate-400">
        Historical prices are provided for
        research purposes and do not guarantee
        future performance.
      </p>
    </section>
  );
